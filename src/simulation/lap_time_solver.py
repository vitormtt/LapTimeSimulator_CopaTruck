...
def run_bicycle_model(params_dict, circuit, config, save_csv=True, out_path=None):
    ...
    if window_size % 2 == 0:
        window_size += 1
    x = savgol_filter(x_raw, window_length=window_size, polyorder=3) if window_size > 3 else x_raw.copy()
    y = savgol_filter(y_raw, window_length=window_size, polyorder=3) if window_size > 3 else y_raw.copy()
    ...
    denom_vec = (truck.mass / radius) - (0.5 * rho * Cl * A_front * mu_aderencia)
    v_lat_max_profile = np.where(
        denom_vec > 0,
        np.sqrt(np.maximum(0.0, (mu_aderencia * truck.mass * g) / np.maximum(denom_vec, 1e-9))),
        250.0 / 3.6,
    )
    ...
    for i in range(1, n):
        ...
        Fz_rear_dynamic = Fz_rear_static + truck.mass * a_prev * (h / L) + F_downforce * 0.5
    ...
    for i in reversed(range(n - 1)):
        ...
        Fz_front_dyn = (truck.mass * g * (lr / L)) + Delta_Fz_brake + F_downforce_next * 0.5
        Fz_rear_dyn  = max(0.0, (truck.mass * g * (lf / L)) - Delta_Fz_brake + F_downforce_next * 0.5)
        ...
        a_decel_brakes = min(
            ...
            truck.brakes.get_max_deceleration(v_next, Fz_front_dyn, Fz_rear_dyn)
        )
    ...
    for i in range(n):
        ...
        if i > 0 and v_profile[i] > 0:
            ...
            potencia_kw    = max(0.0, truck.mass * a_long[i] + F_drag_inst) * v_profile[i] / 1000
            if potencia_kw > 0:
                consumo_acum[i] = consumo_acum[i - 1] + truck.engine.get_fuel_consumption(potencia_kw, dt)
            else:

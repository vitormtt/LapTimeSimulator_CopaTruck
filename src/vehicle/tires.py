...
class ThermalPacejkaTire(PacejkaTire):
    ...
    def __init__(self, config: dict):
        ...
        # Estado Inicial
        self.T_core = config.get('T_initial_C', 65.0) # Temp da carcaça do pneu (C)
        self.T_ambient = config.get('T_ambient_C', 25.0)
        self.P_cold = config.get('P_cold_bar', 4.5)   # Pressão inicial a frio (bar)
        
        # Constantes Térmicas (Aproximadas para pneu de caminhão)
        self.mass_tire = 60.0  # Massa de borracha afetada (kg)
        self.c_p = 1200.0      # Calor específico da borracha (J/kg.K)
        self.h_conv_base = 5.0 # Coeficiente de convecção base
        self.area_cooling = 1.5 # Área de troca térmica do pneu (m2)
    ...
    def update_thermal_state(self, slip_angle_rad: float, fy: float, speed_mps: float, dt: float):
        ...
        self.current_grip_mult = np.clip(grip_mult, 0.75, 1.0) # Não perde mais que 25% de grip
